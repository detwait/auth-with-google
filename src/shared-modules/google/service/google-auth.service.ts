...
export class GoogleAuthService {
  ...
  constructor(private readonly configService: ConfigService<EnvConfigInput>) {
  ...
  async getUserInfo(): Promise<GoogleUserInfoDto> {
    const userInfoClient: any = google.oauth2('v2').userinfo;
    const userInfoResponse: any = await userInfoClient.get({ auth: this.oauthClient });
